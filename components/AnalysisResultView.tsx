           ...
           <div className="flex-1 bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-[2rem] p-8 hover:border-cyan-500/30 transition-colors">
              {mode === AppMode.COMPETITOR_SPY && result.competitorInsights ? (
                 ...
                 <>
                   ...
                   <p className="text-slate-300 text-sm leading-relaxed border-l-2 border-purple-500 pl-4 py-1 font-medium">
                 ...
                 <ResponsiveContainer width="100%" height="100%">
                   <BarChart 
                      data={result.competitorInsights.spyMatrix.map(item => ({
                        ...
                        score: item.impactScore || 50
                      }))}
                      margin={{ top: 10, right: 30, left: 0, bottom: 0 }}
                   >
                     <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
                     <XAxis dataKey="name" stroke="#64748b" fontSize={10} tickLine={false} axisLine={false} dy={10} />
                     <YAxis stroke="#64748b" fontSize={10} tickLine={false} axisLine={false} domain={[0, 100]} dx={-10} />
                   ...
                   <tbody className="text-sm">
                     {result.competitorInsights.spyMatrix.map((row, i) => (
                       <tr key={i} className="group hover:bg-white/5 transition-colors border-b border-slate-800/50 last:border-0">
                         <td className="py-5 pl-4 font-bold text-white group-hover:text-indigo-300 align-top w-1/4">{row.hookUsed}</td>
      ...
      <div className="relative group">
        ...
        <div className="relative bg-black/80 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-10 overflow-hidden shadow-2xl">
          ...
          <div className="mt-10 pt-8 border-t border-white/10 flex flex-wrap gap-4">
             <button onClick={handleSmartShare} className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 rounded-2xl transition-all hover:shadow-[0_0_30px_-5px_rgba(99,102,241,0.4)] hover:-translate-y-1 flex items-center justify-center text-sm tracking-wide">
             ...
             <div className="flex gap-3">
               {[
                 ...
                 <a key={i} href={btn.url} target="_blank" rel="noopener noreferrer" className={`p-4 rounded-2xl transition-all border ${btn.color}`}>
